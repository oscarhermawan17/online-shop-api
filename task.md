...
| #   | Task                           | Description                                                                                                                                                                                                                                                          | SP  | Status |
| --- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ------ |
| 2.1 | Cart management                | Add/remove items, order per unit (PCS). Backend `CartItem` model + full CRUD at `/customer/cart` (get, add, set quantity, remove, clear, merge for guest→login sync). Client-side Zustand syncs with backend on login.                                               | 5   | Done   |
...
| 5.5 | Delivery SLA tracking       | Hourly cron `overdue-delivery.job.ts` finds `shipped` + `deliveryMethod=delivery` orders past 48h; sends WA to admin + in-app notification; sets `overdueNotifiedAt` (once only). Pickup orders excluded. | 5   | Done   |
...
| 8.2 | Login activity chart | Track admin/customer login events to DB; chart in admin dashboard showing login frequency over time                                                                                                                       | 3   | Done   |
...
| 9.3 | WhatsApp notifications | WA via Fonnte. `src/utils/whatsapp.ts`. Config per Store in DB (`fonnteToken`, `fonnteEnabled`, `adminWhatsapp`). Fire-and-forget. **Scope**: order placed → customer + admin ✅; delivery overdue 48h → admin ✅ (cron, delivery only); complaint filed → admin ✅ (delivery only). Other events intentionally excluded (WA cost). | 5   | Done    |
